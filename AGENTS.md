...
核心 Rust 模块已添加单元测试（使用 `tempfile` 创建隔离的临时环境）：

| 模块 | 测试数量 | 覆盖场景 |
|------|---------|----------|
| `scanner.rs` | 6 | 文本/二进制判定、隐藏文件过滤、系统目录跳过、exclude_binary、父子关系、节点数限制 |
| `detector.rs` | 7 | BOM 检测（UTF-8/UTF-16LE/UTF-16BE）、GBK chardet、UTF-8 启发式、空文件、二进制、编码映射表 |
| `converter.rs` | 4 | GBK→UTF-8 内容一致性、UTF-8→UTF-8-BOM、大小校验失败、修改时间保留 |
| `locker.rs` | 3 | 锁定/解锁、重复锁定、批量解锁 |
